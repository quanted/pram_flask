...
RUN cd /src/pram_flask/ubertool && git submodule update --init && git submodule foreach --recursive git brach dev origin/dev && git submodule foreach --recursive git pull origin dev
